...
export default function Profile() {
  ...
  const handleShowReports = async () => {
    try {
      ...
      const res = await fetch(`/server/user/reports/${currentUser._id}`);
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleReportDelete = async (reportId) => {
    try {
      const res = await fetch(`/server/report/delete/${reportId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto mt-10'>
      ...
      <button onClick={handleShowReports} className='text-green-700 w-full'>
        Show Reports
      </button>
      <p className='text-red-700 mt-5'>
        {showReportsError ? 'Error showing reports' : ''}
      </p>
